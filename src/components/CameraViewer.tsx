...
export default function CameraViewer({ camera, onClose, onLocate }: CameraViewerProps) {
  ...
  const isTrafficCamera = camera?.type === 'traffic_camera';
  const brandColor = isTrafficCamera ? '#FF8C00' : '#39FF14';
  const brandLabel = isTrafficCamera ? 'TRAFFIC CAMERA' : 'CCTV CAMERA';
